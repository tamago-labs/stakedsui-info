...
const Details = ({ itemName, gameName, nftType, description, slug }) => {
    ...
    return (
        <>
            ...
            <ul class="text-sm  text-center mt-3 text-gray-500 divide-x divide-gray-200  font-bold   flex  ">
                <li class="w-full">
                    <div onClick={() => setTab(1)} class={`inline-block w-full p-4 text-white border border-r-0 cursor-pointer hover:underline ${tab === 1 && "active underline"}`} aria-current="page">Buy Orders</div>
                </li>
                <li class="w-full">
                    <div onClick={() => setTab(2)} class={`inline-block w-full p-4 border border-l-0 text-white  cursor-pointer hover:underline ${tab === 2 && "active underline"}`}>Sell Orders</div>
                </li>
            </ul>
